...
function createHub(opt) {
  ...
  /** 沒人就立即關房，關房時邀請連結一起失效（§0.3） */
  function closeIfEmpty(room) {
    if (!room || !rooms.has(room.id)) return false;
    if ([...room.members.values()].some(m => m.connected)) return false;
  ...
  function createRoom(person, cfg) {
    ...
    const room = {
      ...
      match: null,
      seed: null,
      inputs: new Map(),              /* personId → { seq, dir } */
      lastSeq: new Map(),
      lastStamp: new Map(),
      chat: [],
      invite: null,
  ...
  function tick(dtMs) {
    ...
    for (const room of rooms.values()) {
      if (room.phase === 'playing' && room.match) {
        const inputs = {};
        for (const m of connectedSeats(room)) {
        ...
        for (let i = 0; i < subs; i++) {
          ...
          if (r.events.length) events = events.concat(r.events);
        ...
        out.push(room);
      } else if (room.phase === 'result' && (now() >= room.resultUntil || allResultDone(room))) {
