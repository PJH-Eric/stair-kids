...
function createHub(opt) {
  ...
  function join(roomId, person, wantRole) {
    ...
    room.members.set(person.id, {
      ...
      role: role,
      /* 這個人的螢幕想看幾格（protocol.js 的 'view'）。開局時全房統一成一個值，
       * 沒回報過的（舊版客戶端）就當作預設值。 */
      viewH: person.viewH || Rules.C.VIEW_H,
      ready: false,
  ...
  /**
   * 這一局全房看幾格。**整間房只能有一個值**：可見高度就是死亡線，
   * 兩邊不一樣等於兩套規則（伺服器也只會用一套，另一邊會看到「還在畫面裡卻死了」）。
   *
   * 取所有上場玩家裡「最想看得深」的那個（通常是手機直向的人）。
   * 取最大而不是最小：取小的話直向那一位的畫面下面又會多出一條誰都用不到的空白，
   * 那正是這整組改動要解掉的東西。代價是橫向的那一位縮放會變小（畫面變窄長）——
   * 那只是視覺，規則兩邊完全一樣，公平不受影響。
   * 觀戰者不算：他們不影響規則，人進人出也不該改變正在打的這一局。
   */
  function matchViewH(seats) {
    let want = Rules.C.VIEW_H;
    for (const m of seats) if (m.viewH > want) want = m.viewH;
    return want;
  }
  ...
  function snapshot(room, opts) {
    ...
    if (opts.full) {
      ...
      /* 鏡像是本地用同一組參數重算出來的，所以死亡線也要一起傳 ——
       * 少了它客戶端會用預設的 16 格重建，畫面下緣跟伺服器的判定線對不上。 */
      snap.viewH = m.viewH;
      snap.steps = m.steps;
