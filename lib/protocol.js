...
/** 客戶端回報的可見高度。外面來的數字一律夾進合法範圍（這是規則參數，不能讓前端亂給）。 */
function setViewH(person, v) {
  const n = Math.round(Number(v));
  if (!Number.isFinite(n) || !n) return;
  person.viewH = Math.max(Rules.C.VIEW_H_MIN, Math.min(Rules.C.VIEW_H_MAX, n));
}
  ...
  function handle(person, msg) {
    ...
    switch (msg.type) {
      ...
      case 'rooms':
        loop.sendLobbyTo(person.id);
        return true;
      /* 這台裝置的螢幕想看幾格（手機直向比較深）。只記下來，下一局開局時全房統一，
       * 對局中收到也不會動到正在打的這一局的死亡線。 */
      case 'view': {
        setViewH(person, msg.viewH);
        if (room && room.members.has(person.id)) room.members.get(person.id).viewH = person.viewH;
        return true;
      }
      ...
      /* 結算看完了。兩邊都看完，tick 就會提早把房間收回大廳（rooms.js 的 resultDone） */
      case 'result-done': {
        if (!room) return false;
        const r = hub.resultDone(room.id, person.id);
        return !r.error;
      }
      case 'input': {
        if (!room) return false;
