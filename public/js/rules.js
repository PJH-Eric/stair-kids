...
(function (root, factory) {
  ...
  const C = {
    ...
    VIEW_H: 16,                  /* 可見高度（格）。18 → 14 是為了把畫面拉近，14 → 16 是因為拉太近了。
                                  * 每一格的像素變大 → 角色變明顯、場地那 12 格的可操作寬度
                                  * 也跟著吃掉更多畫面寬（近正方形視窗實測 66% → 76%）。
                                  * 這條線同時是「掉出畫面就摔死」的死亡線。
                                  * 角色下方看得到幾格＝VIEW_H − CAMERA_LEAD：14 格時只有 5.5 格，
                                  * 速度拉起來之後來不及看清該往哪一側跳；16 格時有 7.5 格（＋36%）。
    ...
    CAMERA_LEAD: 8.5,            /* 鏡頭跟在「最深存活者 − 8.5 格」。
                                  ...
                                  * 收小會讓天花板逼近、下墜速度與回血節奏整組跑掉（實測掉 2 項測試）。
                                  * 想讓角色下方看得更遠，要改 VIEW_H，不要動這一條。 */
