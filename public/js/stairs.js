...
(function (root, factory) {
  ...
  const C = {
    /* 場地寬（格），左右是牆。12 → 16。
   *
   * 為什麼放寬場地就等於放寬畫面比例：每一格的像素大小是
   *   scale = min(畫面寬 ÷ 場地格數, 畫面高 ÷ (可見高度 ＋ 天花板))
   * 在高度吃緊的視窗（大部分的視窗都是）取的是後者，跟場地格數無關，
   * 所以場地多幾格，畫面上的可玩區域就直接變寬，而角色的像素大小完全不變。
   *
   * 可玩區域佔畫面寬的比例（實測）：
   *   場地 12 格：近正方形視窗 76%、平板直向 76%
   *   場地 16 格：近正方形視窗 100%、平板直向 100%、桌機 16:10 約 90%
   *
   * 再往上加就會開始改變玩法（場地越寬，橫向要跑的距離越長），
   * 而且可玩區域的長寬比 16 : 15.1 已經接近正方形 —— 16:9 的視窗左右一定會留白，
   * 那是形狀本身的限制，不是版面沒調好。 */
    FIELD_W: 16.0,
    MIN_PASS: MIN_PASS,
    GAP_MIN: 2.6,         /* 層間垂直間距下限（玩家高 2.0 格，太密會一直被天花板夾） */
    GAP_MAX: 3.6,         /* 層間垂直間距上限 */
    WIDTH_MIN: 3.2,       /* 一般階梯寬下限（場地變寬，階梯跟著等比放大） */
    WIDTH_MAX: 5.0,       /* 一般階梯寬上限 */
    SPAWN_WIDTH: 8.0,     /* 第 0 層出生大平台（唯一的例外層）；兩側各留 4 格＞MIN_PASS */
    WIDE_WIDTH: 6.2,      /* 喘息點的較寬普通階 */
